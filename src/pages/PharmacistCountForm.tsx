...
import { Save, AlertCircle, Loader2, PackageSearch } from 'lucide-react';
...
export default function PharmacistCountForm() {
  const { user } = useAuth();
    ...
    async function fetchInventory() {
      try {
        ...
        snap.forEach(docSnap => {
          loadedItems.push(docSnap.data() as InventoryItem);
        });
  ...
  const handleSave = async (status: 'draft' | 'completed') => {
    ...
    try {
      ...
      handleFirestoreError(err, OperationType.CREATE, 'daily_counts/items');
  ...
  if (items.length === 0) {
    return (
      <div className="bg-white rounded-3xl p-8 text-center border border-gray-100 shadow-sm max-w-2xl mx-auto">
        ...
        <p className="text-gray-500 mb-6">Nenhum produto encontrado. Por favor, importe o CSV primeiro.</p>
  ...
  const filteredItems = items.filter(item => 
    ...
    item.mvCode.toLowerCase().includes(searchTerm.toLowerCase())
  );

  return (
    <div className="max-w-4xl mx-auto pb-24">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 tracking-tight">Contagem do Farmacêutico</h1>
        <p className="text-gray-500 mt-2">
          Insira a quantidade física de cada medicamento de toda a base.
        </p>
      </div>
